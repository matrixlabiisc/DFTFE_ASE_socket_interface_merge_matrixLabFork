...
namespace dftfe
{
  ...
  public:
    ...
    dftfeWrapper(
      ...
      const std::vector<dftfe::uInt> mpGrid = std::vector<dftfe::uInt>(),
      const std::vector<dftfe::uInt> mpGridShift = std::vector<dftfe::uInt>(),
      const dftfe::Int               spinPolarizedDFT = -1,
      const double                   startMagnetization     = -1.0,
      const double                   fermiDiracSmearingTemp = -1.0,
      const dftfe::uInt              npkpt                  = 999999,
      const double                   meshSize               = -1.0,
      const double                   scfMixingParameter     = -1.0,
      const dftfe::Int               polynomialOrder        = -1,
    ...
    reinit(const MPI_Comm                        &mpi_comm_parent,
           ...
           const std::vector<std::vector<double>> cell,
           const std::vector<bool>                pbc,
      const std::vector<dftfe::uInt> mpGrid = std::vector<dftfe::uInt>(), // empty represents not provided
      const std::vector<dftfe::uInt> mpGridShift = std::vector<dftfe::uInt>(),
      const dftfe::Int               spinPolarizedDFT       = -1,
      const double                   startMagnetization     = -1.0,
      const double                   fermiDiracSmearingTemp = -1.0,
      ...
      const double                   meshSize               = -1.0,
      const double                   scfMixingParameter     = -1.0,
      const std::string              mixingScheme           = "Unprovided",
      const dftfe::Int               polynomialOrder        = -1,
      const double                   tolerance              = -1.0,
      const std::string              xc                     = "Unprovided",
      const double                   atomBallRadius         = -1.0,
      const dftfe::uInt              numBands               = 999999,
      ...
      const dftfe::Int  mixingHistory              = -1,
