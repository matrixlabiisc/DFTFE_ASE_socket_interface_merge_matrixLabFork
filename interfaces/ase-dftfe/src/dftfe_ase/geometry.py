...
# Clearance aimed for when a shift is required, in fractional units. Large
# enough that a later relaxation step wandering by a fraction of an Angstrom
# does not immediately re-trip the bound and force another shift.
TARGET_MARGIN = 1e-3
...
def fractional(positions, cell):
    ...
    return np.asarray(positions, float) @ np.linalg.inv(np.asarray(cell, float))
...
def rigid_shift(positions, cell, pbc, *, unit="A"):
    ...
    for axis in needs_shift(positions, cell, pbc):
        lo, hi = frac[:, axis].min(), frac[:, axis].max()
        span = hi - lo
        if span >= 1.0 - 2.0 * TARGET_MARGIN:
