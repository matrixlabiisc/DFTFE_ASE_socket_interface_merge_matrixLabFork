...
SocketDriver::SocketDriver(const std::string& host, int port, MPI_Comm comm)
    : host(host), port(port), comm(comm), sockfd(-1) {
...
void SocketDriver::connect_socket() {
    if (rank == 0) {
...
void SocketDriver::close_socket() {
    if (rank == 0 && sockfd >= 0) {
        close(sockfd);
        sockfd = -1;
    }
...
void SocketDriver::send_data(const std::string& data) {
    if (rank == 0) {
        std::string msg = data + "\n";
...
std::string SocketDriver::receive_data() {
    std::string data;
    if (rank == 0) {
        char buffer[4096];
        bool done = false;
...
std::vector<std::vector<double>> parse_matrix(const std::string& json, const std::string& key) {
    std::vector<std::vector<double>> mat;
    size_t pos = json.find("\"" + key + "\"");
    if (pos == std::string::npos) return mat;
    
    pos = json.find("[", pos); // Start of outer array
    if (pos == std::string::npos) return mat;
    
    size_t end = json.find("]]", pos);
    if (end == std::string::npos) return mat;
    
    std::string content = json.substr(pos + 1, end - pos); // Inside outer brackets
...
std::vector<dftfe::uInt> parse_int_array(const std::string& json, const std::string& key) {
    ...
    size_t end = json.find("]", pos);
    if (end == std::string::npos) return vec;
...
std::vector<bool> parse_bool_array(const std::string& json, const std::string& key) {
    std::vector<bool> vec;
    size_t pos = json.find("\"" + key + "\"");
    if (pos == std::string::npos) return vec;
    ...
    while (std::getline(ss, val, ',')) {
        ...
        if (val == "true") vec.push_back(true);
...
template <typename T>
T parse_scalar(const std::string& json, const std::string& key, T default_val) {
    size_t pos = json.find("\"" + key + "\":");
    ...
    T val;
    if (val_str == "true") return (T)true;
    if (val_str == "false") return (T)false;
    ss >> val;
    return val;
    return val;
}
...
std::string parse_string(const std::string& json, const std::string& key, std::string default_val = "") {
    size_t pos = json.find("\"" + key + "\":");
    if (pos == std::string::npos) return default_val;
    
    pos = json.find(":", pos) + 1;
    size_t start_quote = json.find("\"", pos);
    if (start_quote == std::string::npos) return default_val;
    
    size_t end_quote = json.find("\"", start_quote + 1);
    if (end_quote == std::string::npos) return default_val;
    
    return json.substr(start_quote + 1, end_quote - start_quote - 1);
}

void SocketDriver::parse_request(const std::string& json, 
    ...
    if (mp_grid.empty()) mp_grid = {1, 1, 1};
    
    mp_grid_shift = parse_bool_array(json, "mp_grid_shift");
    if (mp_grid_shift.empty()) mp_grid_shift = {false, false, false};
    
    spin_polarized = parse_scalar<bool>(json, "spin_polarized", false);
    start_magnetization = parse_scalar<double>(json, "start_magnetization", 0.0);
...
std::string SocketDriver::format_response(double energy, 
    ...
    for (size_t i = 0; i < forces.size(); ++i) {
        ...
        if (i < forces.size() - 1) ss << ",";
    }
    ss << "], \"stress\": [";
    for (size_t i = 0; i < stress.size(); ++i) {
        ss << "[" << stress[i][0] << "," << stress[i][1] << "," << stress[i][2] << "]"; // Assuming 3x3 or similar
        if (i < stress.size() - 1) ss << ",";
    }
    ss << "]}";
    return ss.str();
}
...
std::vector<std::vector<double>> invert_3x3(const std::vector<std::vector<double>>& m) {
...
std::vector<std::vector<double>> mat_mul(const std::vector<std::vector<double>>& A, 
...
void SocketDriver::run() {
    ...
    if (rank == 0) std::cout << "SocketDriver: Starting server..." << std::endl;
    
    connect_socket();
    if (rank == 0) std::cout << "SocketDriver: Connected to socket." << std::endl;
    ...
    while (true) {
        if (rank == 0) std::cout << "SocketDriver: Waiting for data..." << std::endl;
        std::string json = receive_data();
        if (rank == 0) std::cout << "SocketDriver: Received data." << std::endl;
        ...
        if (cmd == "exit") {
            if (rank == 0) std::cout << "SocketDriver: Received exit command." << std::endl;
            break;
        }
        
        if (!initialized) {
            // First run: Initialize DFT-FE with explicit data
            if (rank == 0) std::cout << "SocketDriver: Initializing dftfeWrapper with explicit data..." << std::endl;
            ...
            if (new_coords.size() != current_coords.size()) {
                 if (rank == 0) std::cerr << "Atom count mismatch!" << std::endl;
                 break;
            }
            ...
            if (max_disp > 1e-6) {
                if (rank == 0) std::cout << "SocketDriver: Updating atom positions... Max disp: " << max_disp << std::endl;
                dft.updateAtomPositions(displacements);
            } else {
                if (rank == 0) std::cout << "SocketDriver: Displacements small, skipping update." << std::endl;
            }
            
            // Handle cell deformation (simplified for now, similar to before)
            // ... (omitting complex cell deformation logic for this step to focus on basic integration as requested)
            // If needed, we can re-add the deformation logic.
        }
        ...
        send_data(response);
        if (rank == 0) std::cout << "SocketDriver: Response sent." << std::endl;
    }
