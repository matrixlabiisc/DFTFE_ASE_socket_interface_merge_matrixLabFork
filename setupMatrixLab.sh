...
cuda_compiler="/apps/softwares/spack/opt/spack/linux-ubuntu24.04-cascadelake/gcc-13.3.0/cuda-12.6.3-wdrnyla44db6bmihvkefukg5f6axzprg/bin/nvcc"
cxx_flags="-g -std=c++20 -march=native -fopenmp -fPIC" #sets DCMAKE_CXX_FLAGS
cxx_flagsRelease="-O2" #sets DCMAKE_CXX_FLAGS_RELEASE
device_flags="-g -arch=sm_70 -ccbin=mpiicpx"                           #(only applicable for withGPU=ON)
device_architectures="70" # set DCMAKE_CXX_CUDA_ARCHITECTURES 
                           #(only applicable for withGPU=ON)


# Export library paths for BLIS and libflame so the linker finds them
...
function cmake_configure() {
  if [ "$gpuLang" = "cuda" ]; then
    cmake -G Ninja -DCMAKE_EXPORT_COMPILE_COMMANDS=TRUE -DCMAKE_CXX_STANDARD=20 -DCMAKE_CXX_COMPILER=$cxx_compiler -DCMAKE_CUDA_COMPILER=$cuda_compiler -DCMAKE_EXE_LINKER_FLAGS="-L/storage/dftfeDependenciesNoMKL/linAlgLibs/install/lib"\
    ...
    cmake -G Ninja -DCMAKE_EXPORT_COMPILE_COMMANDS=TRUE -DCMAKE_CXX_STANDARD=20 -DCMAKE_CXX_COMPILER=$cxx_compiler\
    -DCMAKE_CXX_FLAGS="$cxx_flags"\
    -DCMAKE_CXX_FLAGS_RELEASE="$cxx_flagsRelease" \
    -DCMAKE_BUILD_TYPE=$build_type -DDEAL_II_DIR=$dealiiDir \
    -DALGLIB_DIR=$alglibDir -DLIBXC_DIR=$libxcDir \
